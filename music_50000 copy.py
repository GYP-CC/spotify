...
SPOTIFY_REDIRECT_URI = 'https://1122-101-235-253-205.ngrok-free.app/callback'
...
def main():
    ...
    for playlist_id in playlist_ids:
        ...
        safe_name = playlist_name.replace('/', '_').replace('\\', '_').replace(' ', '_')
        output_file = f'{safe_name}_playlist.json'

        all_tracks = get_tracks_from_playlist(sp, playlist_id)
        result_data = []
        temp_file = tempfile.NamedTemporaryFile(delete=False, mode='w', encoding='utf-8', suffix='.json')

        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_track = {
                executor.submit(process_track, track, track['artists'][0]['name']): track
        ...
        for idx, future in enumerate(as_completed(future_to_track), start=1):
            ...
            print(f'🎵 [{idx}/{len(all_tracks)}] {name} - {result["spotify"]["artists"][0]["name"]} ({result["status"]})')
        ...
        print(f'✅ 已完成播放列表：{playlist_name}，写入到 {output_file}')
